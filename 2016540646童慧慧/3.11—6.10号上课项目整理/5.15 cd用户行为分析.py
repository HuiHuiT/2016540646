...
# 将购买日期列进行数据类型转换
df['购买日期'] = pd.to_datetime(df.购买日期,format = '%Y%m%d') #Y四位数的日期部分，y表示两位数的日期部分，因为不知道导入的日期格式转换一下日期格式
df['月份'] = df.购买日期.values.astype('datetime64[M]')#增加一列月份显示
print(df.head())
print(df.info())
...
# 每月的总销售额
plt.subplot(221)#把几个图拼凑在一起，这里指两行两列第一个
df.groupby('月份')['订单金额'].sum().plot(fontsize=24)#按月份分组，订单金额，求和运算，fontsize设置字体大小
plt.title('总销售额', fontsize=24)

# 每月的消费次数
plt.subplot(222)
df.groupby('月份')['购买日期'].count().plot(fontsize=24)#按月份分组，购买日期，进行计数运算
...
# 每月的消费人数
plt.subplot(224)
df.groupby('月份')['用户ID'].apply(lambda x: len(x.unique())).plot(fontsize=24)#用户ID是唯一的，出现几次代表购买几次
...
(orderdt_max-orderdt_min).mean()#计算一下平均值
